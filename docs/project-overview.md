...
Fitech Agent 是一个以 `Python 3.11+` 和 `LangGraph` 为基础的多 Agent 宏观研究流水线：它围绕“手动触发一次研究运行”这个核心动作，收集双语宏观与市场驱动新闻，完成归一化、事件抽取、可信度评估、资产影响映射、研究简报生成，并把完整链路写入 SQLite，供回放、审计和 D0 / D1 / D5 复盘使用。
...
配置入口在 `src/fitech_agent/config.py`，默认配置文件是 `config/config.toml`。这一层负责：
...
1. 通过 `config/config.toml` 加载默认配置
2. 离线验证时，可使用 `examples/sample_news.json` 作为 `file` source
