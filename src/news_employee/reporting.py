...
from .models import DailyMarketBrief
from .utils import ensure_directory


def render_markdown(report: DailyMarketBrief) -> str:
    lines = [f"# 全球新闻早盘研究简报 - {report.scheduled_for}", "", "## 隔夜重点"]
    ...
    lines.extend(["", "## 风险情景"])
    lines.extend(f"- {item}" for item in report.risk_scenarios or ["暂无新增风险情景。"])
    lines.extend(["", "## 今日观察清单"])
    lines.extend(f"- {item}" for item in report.watchlist or ["暂无。"])
    lines.extend(["", "## 证据附录"])
...
def save_report_artifacts(report: DailyMarketBrief, report_dir: str) -> tuple[str, str | None, list[str]]:
    bucket = report.scheduled_for[:10].replace("-", "")
    output_dir = ensure_directory(Path(report_dir) / bucket)
    markdown_path = output_dir / f"daily_brief_run_{report.run_id}.md"
    pdf_path = output_dir / f"daily_brief_run_{report.run_id}.pdf"
