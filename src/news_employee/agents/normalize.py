...
LEXICON = {
    "美联储|联储|联邦公开市场委员会": "federal reserve ",
    r"fomc": "federal reserve ",
    "中国人民银行|央行": "pboc ",
    "美国消费者价格指数|cpi": "cpi ",
    "非农就业|nonfarm payrolls?": "nonfarm payrolls ",
    r"石油输出国组织|欧佩克|opec\+": "opec ",
    "刺激政策|稳增长": "china stimulus ",
    "关税": "tariff ",
    "停火|地缘冲突|空袭|袭击": "geopolitics ",
    "降息": "rate cut ",
    "加息": "rate hike ",
    "鹰派": "hawkish ",
    "鸽派": "dovish ",
...
STOPWORDS = {
    ...
    "amid", "after", "china", "market", "macro", "news",
}
...
class NormalizationAgent:
    def run(self, items: list[RawNewsItem]) -> list[NewsCluster]:
        ...
        for cluster_key, grouped_items in grouped.items():
            ordered = sorted(grouped_items, key=lambda item: item.published_at)
            languages = sorted({item.language for item in ordered})
            clusters.append(
                NewsCluster(
